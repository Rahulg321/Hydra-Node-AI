...
const ForumQuestionPage = async (props: {
    ...
    const metrics = {
        upvotes: specificForumQuestion.votes.filter(vote => vote.value === 1).length,
        downvotes: specificForumQuestion.votes.filter(vote => vote.value === -1).length,
    ...
    return <div>
        ...
        <div className='space-y-4 mt-4'>
            {replies.length > 0 ? replies.map(reply => (
                <QuestionReply key={reply.id} timeStamp={reply.createdAt} questionId={questionId} content={reply.content} author={
                    {
...
function QuestionReply({ questionId, content, author, timeStamp }: {
    questionId: string
    content: string
    author: {
        name: string
